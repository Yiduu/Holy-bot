...
module.exports = function mentorRoutes(supabase, requireAuth) {
  ...
  router.get('/', requireAuth, async (req, res) => {
    ...
    let query = supabase
      .from('users')
      .select('telegram_id, anonymous_id, sex, preferred_mentee_sex, accepting_requests, rating, rating_count, avatar_preset, user_settings(bio, specialization, max_mentees, display_name)')
      .eq('role', 'mentor')
      .eq('is_banned', false);
    ...
    const enriched = await Promise.all((data || []).map(async (mentor) => {
      ...
      return {
        ...mentor,
        avatar_preset: mentor.avatar_preset || null,
